    ...
    public class DeliveryController : Controller
    {
        ...
        public IActionResult GET()
        {
            try
            {
                ...
                string Query = @"select distinct SO.SalsDelId,SO.BPId,BPM.BPName , SO.BPContPerId  , concat(BPCD.fname,' ', BPCD.Lname ) as BPContactPerName ,
                                ...
                                SO.ConsolidatingBPId,
								SO.ConsolidationType,
                                ...
                                from Sales_Delivery SO  With (NOLOCK)
...
left outer join Project_Mst PM on PM.PrjId = SO.PrjId and PM.IsActive ='Y' where SO.DraftFlag ='Y'";
        ...
        public IActionResult GETITEMDET(string ID, string Flag)
        {
            ...
            {
                string ConnectionString = HttpContext.Session.GetString("ConnectionString");
                string Query = @"select distinct SD.SalsDelDetId , SD.SalsDelId , SD.ItemId ,SD.LineNum , SD.Qty ,SD.InfoPrice ,
				SD.DisPer ,SD.TaxCodeId ,SD.TotalLC ,SD.CalcTotalLC ,SD.UomId ,SD.CntryId ,SD.WhsId , SD.LocName ,
				SD.BlKAId ,SD.ItemBrchId ,SD.ItemHSNNo ,
				SD.ServiceId ,SD.TaxAmountLC ,SD.BaseDocType ,SD.BaseDocEntry ,SD.BaseDocLineNum ,
				SD.CreditMemoQty ,SD.GLAcctName ,SD.BarCode ,SD.Remarks ,SD.CreatedBy ,SD.CreateDate ,SD.CreateTS ,SD.UpdatedBy ,
				SD.UpdateDate ,SD.UpdateTS ,SD.OpenQty ,SD.RowStatus ,SD.ActualPrice ,SD.PriceUpd ,SD.PrevQty ,'N' as Editflag,
				IM.ItemName , TCM.TaxCode ,UM.UomCode , UM.UomName ,CM.CntryCurrCode , WHM.WhsName , BM.BrchName ,SM.ServiceName ,IBD.BarCode 
                from Sales_Delivery_det SD With (NOLOCK)
                left outer join Item_Mst IM on IM.ItemId = SD.ItemId and IM.IsActive ='Y'
                left outer join TaxCode_Mst TCM on TCM.TaxCodeId = SD.TaxCodeId and TCM.IsActive ='Y'
                left outer join Unit_Mst UM on UM.UomId = SD.UomId and UM.IsActive ='Y'
                left outer join Country_Mst CM on CM.CntryId = SD.CntryId and CM.IsActive ='Y'
                left outer join WareHouse_Mst WHM on WHM.WhsId = SD.WhsId and WHM.IsActive ='Y'
                left outer join Branch_Mst BM on BM.BrchId = SD.ItemBrchId and BM.IsActive ='Y'
                left outer join SAC_Mst SM on SM.ServiceId = SD.ServiceId and SM.IsActive ='Y'
                left outer join Item_BarCode_det IBD on IBD.ItemId = SD.ItemId and IBD.IsActive ='Y'
                where SD.SalsDelId =' " + ID + "' and SD.RowStatus in ('" + Flag + "','O','T')";
                List<Dictionary<string, object>> dataList = new List<Dictionary<string, object>>();
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    using (SqlCommand cmd = new SqlCommand(Query, con))
...
FROM Sales_Delivery_Fret sof
INNER JOIN Freight_Mst fm ON sof.FretId = fm.FretId
WHERE sof.SalsDelId ='" + Id + "'";
                List<Dictionary<string, object>> dataList = new List<Dictionary<string, object>>();
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    using (SqlCommand cmd = new SqlCommand(Query, con))
            ...
            try
            {
                ...
                if (flag == "T")
                {
                    Query = @"select distinct SOT.*,TCM.TaxCode , TCM.TaxFormula , TCM.TaxCalRate 
                        from Sales_Delivery_Tax SOT
                        inner join Sales_Delivery SQ on SQ.SalsDelId = SOT.SalsDelId
                        inner join Sales_Delivery_det SOD on SOD.SalsDelId = SQ.SalsDelId 
                        ...
                        and TCM.IsActive ='Y'
                        where SOT.SalsDelId = '" + id + "' And SOT.LineNum='" + lineNum + "' And SOT.Tax_type ='" + flag + "' ";
                }
                else
                {
                    Query = @"   select distinct SOT.*,TCM.TaxCode , TCM.TaxFormula , TCM.TaxCalRate 
            ...
            try
            {
                ...
                switch (Flag)
                {
                    ...
                    case "SBA":
                        ...
                        break;
                    case "SQ":
                        Query = @"Select SQ.* ,BPM.BPName ,concat(EM.EmpFname ,' ', EM.EmpLname) as EmpName
                                From Sales_Quot  SQ
                                inner join BP_Mst BPM on BPM.BPId = SQ.BPId and BPM.IsActive ='Y'
                                inner join EmployeeMst EM on EM.EmpId = SQ.EmpId and EM.IsActive ='Y'
                                where SQ.DraftFlag ='N' And SQ.Status='O' And SQ.BPId  ='" + BPId + "'";
                        break;
                    case "SO":
                        Query = @"
                        Select SO.* ,BPM.BPName
                                From Sales_Order SO
                                inner join BP_Mst BPM on BPM.BPId = SO.BPId and BPM.IsActive = 'Y'
        ...
        public IActionResult GETTAXDATA(string SalsDelId)
        {
            try
            {
                string ConnectionString = HttpContext.Session.GetString("ConnectionString");
                string Query = @"SELECT * FROM [Sales_Delivery_Tax_det] where SalsDelId='" + SalsDelId + "'";
                List<Dictionary<string, object>> dataList = new List<Dictionary<string, object>>();
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    using (SqlCommand cmd = new SqlCommand(Query, con))
        ...
        [HttpPost]
        public IActionResult POSTUPDATE(Delivery_Mst item)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            string Lastid = "";
            try
            {
                ...
                if (item.SalsDelId != "" && item.SalsDelId != null)
                {
                    ...
                    string Query = genrate.GenerateUpdateQuery(item, "[Sales_Delivery]", "SalsDelId", item.SalsDelId, "");
                ...
                else
                {
                    ...
                    if (item.BPId != null || item.BPId != "")
                    {
                        string insertQuery = genrate.GenerateInsertQuery(item, "[Sales_Delivery]", "SalsDelId");
                        using (SqlConnection con = new SqlConnection(ConnectionString))
                        {
                            con.Open();
                            using (SqlCommand cmd = new SqlCommand(insertQuery, con))
                            {
                                cmd.CommandTimeout = 300;
                                cmd.ExecuteNonQuery();
                                cmd.CommandText = "SELECT MAX(SalsDelId) AS SalsDelId FROM Sales_Delivery;";
                                SqlDataReader rdr = cmd.ExecuteReader();
                                {
                                    while (rdr.Read())
                                    {
                                        Lastid = rdr["SalsDelId"].ToString();
                                    }
                                }
                            }
                ...
                if (item.DraftFlag == "Y")
                {
                    return Json(new { Success = true, LastId = Lastid, Message = "Delivery Drafted Successfully..!" });
                }
                else
                {
                    return Json(new { Success = true, LastId = Lastid, Message = "Delivery Added Successfully..!" });
                };
            }
            catch (Exception ex)
            {
...
FROM Sales_Delivery 
inner join Finyr_mst on Sales_Delivery.FinyrId = Finyr_mst.FinyrId 
and Finyr_mst.IsActive ='Y'   where DraftFlag='" + flag + "'; ";
        ...
        public IActionResult ADDITEMATTACHMENT([FromBody] List<DeliveryAttchment> Data)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
        ...
        public IActionResult ADDITEMDETAILS([FromBody] List<Delivery_Item> Data)
         {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
                ...
                foreach (var item in Data)
                {
                    ...
                    if (item.SalsDelDetId != "" && item.SalsDelDetId != null)
                    {
                        item.UpdateDate = DateOnly.FromDateTime(DateTime.Now.Date);
                        if (item.RowStatus == "O")
                        {
                        ...
                        item.UpdatedBy = HttpContext.Session.GetString("UserName");
                        Genrate_Query genrate = new Genrate_Query();
                        string Query = genrate.GenerateUpdateQuery(item, "[Sales_Delivery_det]", "SalsDelDetId", item.SalsDelDetId, "");
                        if (item.SalsDelId != null || item.SalsDelId != "")
                        {
                            using (SqlConnection con = new SqlConnection(ConnectionString))
                            {
                                con.Open();
                    ...
                    else
                    {
                        ...
                        if (item.SalsDelId != null || item.SalsDelId != "")
                        {
                            string insertQuery = genrate.GenerateInsertQuery(item, "[Sales_Delivery_det]", "SalsDelDetId");
                            using (SqlConnection con = new SqlConnection(ConnectionString))
                            {
                                con.Open();
                                using (SqlCommand cmd = new SqlCommand(insertQuery, con))
                ...
                ADDTAX(Data[0].SalsDelId, "I");
                ADDSTOCKQTY(Data[0].SalsDelId, "SD", Data[0].SalsDelDetId == null ? "Y" : "N");
                COPYFROMSTATUSCHECK(Data[0].SalsDelId);
                return Json(new { Success = true, Message = "Delivery Item Details Added  Successfully..!" });
            }
            catch (Exception ex)
            {
                return StatusCode(500, ex.Message.ToString());
        ...
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            string Query = @"exec[dbo].[usp_UpdItemWhsStock] 'Sales_Delivery','Sales_Delivery_det','SalsDelId','" + ID + "','" + Flag + "','" + add_flag + "'";
            if (ID != "")
            {
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
        ...
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            string Query = @"exec  [dbo].[usp_UpdBaseDocData] 'Sales_Delivery','Sales_Delivery_det','SalsDelId','" + ID + "' ";
            if (ID != "")
            {
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
        ...
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            string Query = @"exec [dbo].[usp_UpdPurTax] 'Sales_Delivery_det','SalsDelId','Sales_Delivery_Tax','SalsDelTaxId','Sales_Delivery_Fret','SalsDelFretId','" + ID + "','" + Flag + "'";
            if (ID != "")
            {
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
        ...
        public IActionResult ADDFREIGHT([FromBody] List<Delivery_Freight> Data)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
                ...
                foreach (var item in Data)
                {
                    if (item.SalsDelFretId != "" && item.SalsDelFretId != null)
                    {
                        ...
                        Genrate_Query genrate = new Genrate_Query();
                        string Query = genrate.GenerateUpdateQuery(item, "[Sales_Delivery_Fret]", "SalsDelFretId", item.SalsDelFretId, "");
                        if (item.SalsDelId != null || item.SalsDelId != "")
                        {
                            using (SqlConnection con = new SqlConnection(ConnectionString))
                            {
                    ...
                    else
                    {
                        ...
                        if (item.SalsDelId != null || item.SalsDelId != "")
                        {
                            string insertQuery = genrate.GenerateInsertQuery(item, "[Sales_Delivery_Fret]", "SalsDelFretId");
                            using (SqlConnection con = new SqlConnection(ConnectionString))
                            {
                                con.Open();
                                using (SqlCommand cmd = new SqlCommand(insertQuery, con))
        ...
        public IActionResult DELETEMAINDATA(string Id)
        {
            ...
            try
            {
                ...
                string Query = "Delete from [Sales_Delivery] where SalsDelId='" + Id + "'";
                List<Dictionary<string, object>> dataList = new List<Dictionary<string, object>>();
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    using (SqlCommand cmd = new SqlCommand(Query, con))
        ...
        public IActionResult DELETEDETAILS(string Id)
        {
            ...
            try
            {
                ...
                string Query = "Delete from [Sales_Delivery_det] where SalsDelDetId ='" + Id + "'";
                List<Dictionary<string, object>> dataList = new List<Dictionary<string, object>>();
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    using (SqlCommand cmd = new SqlCommand(Query, con))
        ...
        public IActionResult ADDTAXINFO([FromBody] DeliveryTax_Mst item)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
                ...
                if (!string.IsNullOrEmpty(item.SalsDelTaxDetId))
                {
                    ...
                    item.CreatedBy = HttpContext.Session.GetString("UserName");
                    Genrate_Query genrate = new Genrate_Query();
                    string Query = genrate.GenerateUpdateQuery(item, "[Sales_Delivery_Tax_det]", "SalsDelTaxDetId", item.SalsDelTaxDetId, "");
                    if (!string.IsNullOrEmpty(item.SalsDelId))
                    {
                        using (SqlConnection con = new SqlConnection(ConnectionString))
                        {
                            con.Open();
                ...
                else
                {
                    ...
                    if (!string.IsNullOrEmpty(item.SalsDelId))
                    {
                        string insertQuery = genrate.GenerateInsertQuery(item, "[Sales_Delivery_Tax_det]", "SalsDelTaxDetId");
                        using (SqlConnection con = new SqlConnection(ConnectionString))
                        {
                            con.Open();
                            using (SqlCommand cmd = new SqlCommand(insertQuery, con))
