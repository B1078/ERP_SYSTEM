    ...
    public class EmployeeMasterController : Controller
    {
        ...
        public IActionResult GETDATA()
        {
            try
            {
                ...
                string Query = @"select EM.*,CM.CntryName as PerCntryName , SM.StateName as PerStatName,
CM1.CntryName as CommCntryName, SM1.StateName as CommStateName,DM.DeptName 
from EmployeeMst EM
inner join Country_Mst CM on CM.CntryId = EM.EmpPCntryId 
inner join State_Mst SM on SM.StateId = EM.EmpPStateId 
inner join Dept_Mst DM on DM.DeptId = EM.DeptId 
inner join Country_Mst CM1 on CM1.CntryId = EM.EmpCCntryId 
inner join State_Mst SM1 on SM1.StateId = EM.EmpCStateId ";
        ...
        public IActionResult POSTDATA(EmployeeMaster Data)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
                ...
                string insertQuery = genrate.GenerateInsertQuery(Data, "[EmployeeMst]", "EmpId");
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    con.Open();
                ...
                return Json(new { Success = true, Message = "Employee Details  Added Successfully..!" });
            }
            catch (SqlException sqlEx)
            {
                return StatusCode(500, "A Employee Details with this Employee Code already exists. Please use a different Employee Code.");
            }
            catch (Exception ex)
            {
        ...
        public ActionResult POSTDATAExcel(string data)
        {
            ...
            try
            {
                ...
                List<EmployeeMaster> units = JsonConvert.DeserializeObject<List<EmployeeMaster>>(data);
                ...
                using (SqlConnection con = new SqlConnection(connectionString))
                {
                    ...
                    foreach (var unit in units)
                    {
                        try
                        {
                            ...
                            string insertQuery = genrate.GenerateInsertQuery(unit, "[EmployeeMst]", "EmpId");
        ...
        public IActionResult UPDATEDATA(EmployeeMaster Data)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
                ...
                Data.UpdatedBy = HttpContext.Session.GetString("UserName");
                Genrate_Query genrate = new Genrate_Query();
                string Query = genrate.GenerateUpdateQuery(Data, "[EmployeeMst]", "EmpId", Data.EmpId, "");
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    con.Open();
        ...
        public IActionResult DELETE(string Id)
        {
            ...
            try
            {
                ...
                string Query = "Delete from [EmployeeMst] where EmpId='" + Id + "'";
                List<Dictionary<string, object>> dataList = new List<Dictionary<string, object>>();
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    using (SqlCommand cmd = new SqlCommand(Query, con))
                ...
                return Json(new { success = true, message = "Employee Details Deleted Successfully..!" });
            }
            catch (Exception ex)
            {
                return StatusCode(500, ex.Message.ToString());
