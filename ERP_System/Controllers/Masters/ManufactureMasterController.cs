...
namespace ERP_System.Controllers.Masters
{
    ...
    {
        public IActionResult ManufactureMaster()
        {
        ...
        public IActionResult GET()
        {
            try
            {
                ...
                string Query = @"SELECT * from [Manufacture_mst]";
        ...
        [HttpPost]
        public IActionResult POSTDATA(Manufacture_Mst Data)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
                ...
                string insertQuery = genrate.GenerateInsertQuery(Data, "[Manufacture_mst]", "ManuftId");
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    con.Open();
                ...
                return Json(new { Success = true, Message = "Manufacturer Master  Added Successfully..!" });
            }
            catch (SqlException sqlEx)
            {
                return StatusCode(500, "A Manufacturer  Master  with this Manufacturer Code already exists. Please use a different Code.");
            }
            catch (Exception ex)
            {
        ...
        public ActionResult BULKPOST(string data)
        {
            ...
            try
            {
                ...
                List<Manufacture_Mst> units = JsonConvert.DeserializeObject<List<Manufacture_Mst>>(data);
                ...
                using (SqlConnection con = new SqlConnection(connectionString))
                {
                    ...
                    foreach (var unit in units)
                    {
                        try
                        {
                            ...
                            string insertQuery = genrate.GenerateInsertQuery(unit, "[Manufacture_mst]", "ManuftId");
        ...
        public IActionResult UPDATEDATA(Manufacture_Mst Data)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
                ...
                Data.UpdatedBy = HttpContext.Session.GetString("UserName");
                Genrate_Query genrate = new Genrate_Query();
                string Query = genrate.GenerateUpdateQuery(Data, "[Manufacture_mst]", "ManuftId", Data.ManuftId, "");
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    con.Open();
                ...
                return Json(new { Success = true, Message = " Manufacturer Master Updated Successfully..!" });
            }
            catch (Exception ex)
            {
                return StatusCode(500, ex.Message.ToString());
        ...
        public IActionResult DELETE(string Id)
        {
            ...
            try
            {
                ...
                string Query = "Delete from [Manufacture_mst] where ManuftId='" + Id + "'";
                List<Dictionary<string, object>> dataList = new List<Dictionary<string, object>>();
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    using (SqlCommand cmd = new SqlCommand(Query, con))
                ...
                return Json(new { success = true, message = "Manufacturer Master Deleted Successfully..!" });
            }
            catch (Exception ex)
            {
                return StatusCode(500, ex.Message.ToString());
