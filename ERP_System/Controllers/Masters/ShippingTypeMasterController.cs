...
{
    public class ShippingTypeMasterController : Controller
    {
        public IActionResult ShippingTypeMaster()
        {
        ...
        public IActionResult GET()
        {
            try
            {
                ...
                string Query = @"Select * From [ShippingType_mst]";
        ...
        public IActionResult POSTDATA(ShippingType_Mst Data)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
                ...
                string insertQuery = genrate.GenerateInsertQuery(Data, "[ShippingType_mst]", "ShipTypeId");
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    con.Open();
                ...
                return Json(new { Success = true, Message = "Shipping Type Added Successfully..!" });
            }
            catch (SqlException sqlEx)
            {
                return StatusCode(500, "A Shipping Type with this Ship Type Namealready exists. Please use a different Ship Type Name.");
            }
            catch (Exception ex)
            {
        ...
        public ActionResult BULKPOST(string data)
        {
            ...
            try
            {
                ...
                List<ShippingType_Mst> units = JsonConvert.DeserializeObject<List<ShippingType_Mst>>(data);
                ...
                using (SqlConnection con = new SqlConnection(connectionString))
                {
                    ...
                    foreach (var unit in units)
                    {
                        try
                        {
                            ...
                            string insertQuery = genrate.GenerateInsertQuery(unit, "[ShippingType_mst]", "ShipTypeId");
        ...
        public IActionResult UPDATEDATA(ShippingType_Mst Data)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
                ...
                Data.UpdatedBy = HttpContext.Session.GetString("UserName");
                Genrate_Query genrate = new Genrate_Query();
                string Query = genrate.GenerateUpdateQuery(Data, "[ShippingType_mst]", "ShipTypeId", Data.ShipTypeId, "");
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    con.Open();
                ...
                return Json(new { Success = true, Message = "Shipping Type Updated Successfully..!" });
            }
            catch (Exception ex)
            {
                return StatusCode(500, ex.Message.ToString());
        ...
        public IActionResult DELETE(string Id)
        {
            ...
            try
            {
                ...
                string Query = "Delete from [ShippingType_mst] where ShipTypeId='" + Id + "'";
                List<Dictionary<string, object>> dataList = new List<Dictionary<string, object>>();
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    using (SqlCommand cmd = new SqlCommand(Query, con))
                ...
                return Json(new { success = true, message = "Shipping Type Deleted Successfully..!" });
            }
            catch (Exception ex)
            {
                return StatusCode(500, ex.Message.ToString());
