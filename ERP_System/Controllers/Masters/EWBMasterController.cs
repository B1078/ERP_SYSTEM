...
{
    public class EWBMasterController : Controller
    {
        public IActionResult EWBMaster()
        {
        ...
        public IActionResult GETDATA()
        {
            try
            {
                ...
                string Query = @"Select * From [EWBUnit_Mst] WITH (NOLOCK)";
        ...
        public ActionResult POSTDATAExcel(string data)
        {
            ...
            try
            {
                ...
                List<EWBMst> units = JsonConvert.DeserializeObject<List<EWBMst>>(data);
                ...
                using (SqlConnection con = new SqlConnection(connectionString))
                {
                    ...
                    foreach (var unit in units)
                    {
                        try
                        {
                            ...
                            string insertQuery = genrate.GenerateInsertQuery(unit, "[EWBUnit_Mst]", "EWBUomId");
        ...
        public IActionResult POSTDATA(EWBMst Data)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
                ...
                Genrate_Query genrate = new Genrate_Query();
                string insertQuery = genrate.GenerateInsertQuery(Data, "[EWBUnit_Mst]", "EWBUomId");
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    con.Open();
                    using (SqlCommand cmd = new SqlCommand(insertQuery, con))
                ...
                return Json(new { Success = true, Message = " EWB Unit  Added Successfully..!" });
            }
            catch (SqlException sqlEx)
            {
                return StatusCode(500, "A EWB Unit with this EWB Code already exists. Please use a different EWB Code.");
            }
            catch (Exception ex)
            {
        ...
        public IActionResult UPDATEDATA(EWBMst Data)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
                ...
                Data.UpdatedBy = HttpContext.Session.GetString("UserName");
                Genrate_Query genrate = new Genrate_Query();
                string Query = genrate.GenerateUpdateQuery(Data, "[EWBUnit_Mst]", "EWBUomId", Data.EWBUomId, "");
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    con.Open();
                ...
                return Json(new { Success = true, Message = " EWB Unit Updated Successfully..!" });
            }
            catch (Exception ex)
            {
                return StatusCode(500, ex.Message.ToString());
        ...
        public IActionResult DELETE(string Id)
        {
            ...
            try
            {
                ...
                string Query = "Delete from [EWBUnit_Mst] where EWBUomId='" + Id + "'";
                List<Dictionary<string, object>> dataList = new List<Dictionary<string, object>>();
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    using (SqlCommand cmd = new SqlCommand(Query, con))
                ...
                return Json(new { success = true, message = "EWB Unit Deleted Successfully..!" });
            }
            catch (Exception ex)
            {
                return StatusCode(500, ex.Message.ToString());
