...
namespace ERP_System.Controllers.Masters
{
    public class CountryMasterController : Controller
    {
        ...
        public CountryMasterController(ILogger<CountryMasterController> logger, IConfiguration configuration)
        {
            _logger = logger;
        ...
        public IActionResult GET()
        {
            try
            {
                ...
                string Query = @"Select * From [Country_Mst] WITH (NOLOCK)";
        ...
        public ActionResult POSTDATAExcel(string data)
        {
            ...
            try
            {
                ...
                List<CountryMaster> units = JsonConvert.DeserializeObject<List<CountryMaster>>(data);
                ...
                using (SqlConnection con = new SqlConnection(connectionString))
                {
                    ...
                    foreach (var unit in units)
                    {
                        try
                        {
                            ...
                            string insertQuery = genrate.GenerateInsertQuery(unit, "[Country_Mst]", "CntryId");
        ...
        public IActionResult POSTDATA(CountryMaster Data)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
                ...
                Genrate_Query genrate = new Genrate_Query();
                string insertQuery = genrate.GenerateInsertQuery(Data, "[Country_Mst]", "CntryId");
                insertQuery = insertQuery.Replace("'₹'", "N'₹'");
                ...
                if (responce) 
                {
                    ...
                    return Json(new { Success = true, Message = " Country Details Added Successfully..!" });
                }
                else
                {
                    return Json(new { Success = false, Message = "Error To Add Country Details..!" });
                }
                
            }
            catch (SqlException sqlEx)
            {
                return StatusCode(500, "A Country Details  with this Country  already exists. Please use a different Country.");
            }
            catch (Exception ex)
            {
        ...
        public IActionResult UPDATEDATA(CountryMaster Data)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
                ...
                Data.UpdatedBy = HttpContext.Session.GetString("UserName");
                Genrate_Query genrate = new Genrate_Query();
                string Query = genrate.GenerateUpdateQuery(Data, "[Country_Mst]", "CntryId", Data.CntryId, "");
                ...
                if (responce)
                {
                    ...
                    return Json(new { Success = true, Message = " Country Details Updated Successfully..!" });
                }
                else { 
                    return Json(new { Success = false, Message = "Error To Update Country Details ..!" });
        ...
        public IActionResult DELETE(string Id)
        {
            ...
            try
            {
                ...
                string Query = "Delete from [Country_Mst] where CntryId='" + Id + "'";
                var responce = MainDbADD(Query);
                if (responce)
                {
                    using (SqlConnection con = new SqlConnection(ConnectionString))
                    ...
                    return Json(new { success = true, message = "Country Details Deleted Successfully..!" });
                } 
                else 
                {
                    return Json(new { success = false, message = "Error To Deleted Country Details..!" });
                }
