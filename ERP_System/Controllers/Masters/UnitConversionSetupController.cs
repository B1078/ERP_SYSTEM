...
namespace ERP_System.Controllers.Master
{
    public class UnitConversionSetupController : Controller
    {
        public IActionResult UnitConversionSetup()
        {
        ...
        public IActionResult GETDATA()
        {
            try
            {
                ...
                string Query = @"select UCS.*,UM.UomCode as FromUOMCode, UM.UomName  as FromUomName,
UM1.UomCode as ToUOMCode , um1.UomName as ToUOMName
from Uom_Conv_Setup UCS WITH (NOLOCK)
inner join Unit_Mst UM on UM.UomId = UCS.FromUom 
inner join Unit_Mst UM1 on UM1.UomId = UCS.ToUom ";
        ...
        public ActionResult POSTDATAExcel(string data)
        {
            ...
            try
            {
                ...
                List<UnitConversionSetup> units = JsonConvert.DeserializeObject<List<UnitConversionSetup>>(data);
                ...
                using (SqlConnection con = new SqlConnection(connectionString))
                {
                    ...
                    foreach (var unit in units)
                    {
                        try
                        {
                            ...
                            string insertQuery = genrate.GenerateInsertQuery(unit, "[Uom_Conv_Setup]", "SetupId");
        ...
        public IActionResult POSTDATA(UnitConversionSetup Data)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
                ...
                Genrate_Query genrate = new Genrate_Query();
                string insertQuery = genrate.GenerateInsertQuery(Data, "[Uom_Conv_Setup]", "SetupId");
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    con.Open();
                    using (SqlCommand cmd = new SqlCommand(insertQuery, con))
                ...
                return Json(new { Success = true, Message = " Uom_Conv_Setup  Added Successfully..!" });
            }
            catch (Exception ex)
            {
                return StatusCode(500, ex.Message.ToString());
        ...
        public IActionResult UPDATEDATA(UnitConversionSetup Data)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
                ...
                Data.UpdatedBy = HttpContext.Session.GetString("UserName");
                Genrate_Query genrate = new Genrate_Query();
                string Query = genrate.GenerateUpdateQuery(Data, "[Uom_Conv_Setup]", "SetupId", Data.SetupId, "");
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    con.Open();
                ...
                return Json(new { Success = true, Message = " Uom_Conv_Setup Updated Successfully..!" });
            }
            catch (Exception ex)
            {
                return StatusCode(500, ex.Message.ToString());
        ...
        public IActionResult DELETE(string Id)
        {
            ...
            try
            {
                ...
                string Query = "Delete from [Uom_Conv_Setup] where SetupId='" + Id + "'";
                List<Dictionary<string, object>> dataList = new List<Dictionary<string, object>>();
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    using (SqlCommand cmd = new SqlCommand(Query, con))
                ...
                return Json(new { success = true, message = "Uom_Conv_Setup Deleted Successfully..!" });
            }
            catch (Exception ex)
            {
                return StatusCode(500, ex.Message.ToString());
