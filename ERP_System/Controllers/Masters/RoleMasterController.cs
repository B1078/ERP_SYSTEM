    ...
    public class RoleMasterController : Controller
    {
        ...
        public IActionResult GET()
        {
            try
            {
                ...
                string Query = @"Select * From [Role_mst]";
        ...
        [HttpPost]
        public IActionResult POSTDATA(Role_Mst Data)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
                ...
                string insertQuery = genrate.GenerateInsertQuery(Data, "[Role_mst]", "Roleid");
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    con.Open();
                ...
                return Json(new { Success = true, Message = "Role Details Added Successfully..!" });
            }
            catch (SqlException sqlEx)
            {
                return StatusCode(500, "A Role Details with this Role Name already exists. Please use a different Role Name.");
            }
            catch (Exception ex)
            {
        ...
        public ActionResult BULKPOST(string data)
        {
            ...
            try
            {
                ...
                List<Role_Mst> units = JsonConvert.DeserializeObject<List<Role_Mst>>(data);
                ...
                using (SqlConnection con = new SqlConnection(connectionString))
                {
                    ...
                    foreach (var unit in units)
                    {
                        try
                        {
                            ...
                            string insertQuery = genrate.GenerateInsertQuery(unit, "[Role_mst]", "Roleid");
                            using (SqlCommand cmd = new SqlCommand(insertQuery, con))
                            {
        ...
        public IActionResult UPDATEDATA(Role_Mst Data)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
                ...
                Data.UpdatedBy = HttpContext.Session.GetString("UserName");
                Genrate_Query genrate = new Genrate_Query();
                string Query = genrate.GenerateUpdateQuery(Data, "[Role_mst]", "Roleid", Data.Roleid, "");
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    con.Open();
        ...
        public IActionResult DELETE(string Id)
        {
            ...
            try
            {
                ...
                string Query = "Delete from [Role_mst] where Roleid='" + Id + "'";
                List<Dictionary<string, object>> dataList = new List<Dictionary<string, object>>();
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    using (SqlCommand cmd = new SqlCommand(Query, con))
                ...
                return Json(new { success = true, message = "Role Details Deleted Successfully..!" });
            }
            catch (Exception ex)
            {
                return StatusCode(500, ex.Message.ToString());
