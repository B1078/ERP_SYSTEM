...
{
	public class BarCodeMasterController : Controller
	{
		public IActionResult BarCodeMaster()
		{
        ...
        public IActionResult GET()
        {
            try
            {
                ...
                string Query = @"Select * From [BarCode_Mst]";
        ...
        public IActionResult POSTDATA(BarCodeMaster Data)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
                ...
                string insertQuery = genrate.GenerateInsertQuery(Data, "[BarCode_Mst]", "BarCodeId");
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    con.Open();
                ...
                return Json(new { Success = true, Message = "Barcode Added Successfully..!" });
            }
            catch (Exception ex)
            {
                return StatusCode(500, ex.Message.ToString());
        ...
        public ActionResult POSTDATAExcel(string data)
        {
            ...
            try
            {
                ...
                List<BarCodeMaster> units = JsonConvert.DeserializeObject<List<BarCodeMaster>>(data);
                ...
                using (SqlConnection con = new SqlConnection(connectionString))
                {
                    ...
                    foreach (var unit in units)
                    {
                        try
                        {
                            ...
                            string insertQuery = genrate.GenerateInsertQuery(unit, "[BarCode_Mst]", "BarCodeId");
        ...
        public IActionResult UPDATEDATA(BarCodeMaster Data)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
                ...
                Data.UpdatedBy = HttpContext.Session.GetString("UserName");
                Genrate_Query genrate = new Genrate_Query();
                string Query = genrate.GenerateUpdateQuery(Data, "[BarCode_Mst]", "BarCodeId", Data.BarCodeId, "");
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    con.Open();
                    using (SqlCommand cmd = new SqlCommand(Query, con))
        ...
        public IActionResult DELETE(string Id)
        {
            ...
            try
            {
                ...
                string Query = "Delete from [BarCode_Mst] where BarCodeId='" + Id + "'";
                List<Dictionary<string, object>> dataList = new List<Dictionary<string, object>>();
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    using (SqlCommand cmd = new SqlCommand(Query, con))
