...
{
    public class SACSetupController : Controller
    {
        public IActionResult SACSetup()
        {
        ...
        public IActionResult GETDATA()
        {
            try
            {
                ...
                string Query = @"select * from[SAC_Mst]";
        ...
        public ActionResult POSTDATAExcel(string data)
        {
            ...
            try
            {
                ...
                List<SAC_Mst> units = JsonConvert.DeserializeObject<List<SAC_Mst>>(data);
                ...
                using (SqlConnection con = new SqlConnection(connectionString))
                {
                    ...
                    foreach (var unit in units)
                    {
                        try
                        {
                            ...
                            string insertQuery = genrate.GenerateInsertQuery(unit, "[SAC_Mst]", "ServiceId");
        ...
        public IActionResult POSTDATA(SAC_Mst Data)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
                ...
                Genrate_Query genrate = new Genrate_Query();
                string insertQuery = genrate.GenerateInsertQuery(Data, "[SAC_Mst]", "ServiceId");
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    con.Open();
                    using (SqlCommand cmd = new SqlCommand(insertQuery, con))
                ...
                return Json(new { Success = true, Message = " SAC Setup Added Successfully..!" });
            }
            catch (SqlException sqlEx)
            {
                return StatusCode(500, "A SAC Setup with this Service code already exists. Please use a different Service code.");
            }
            catch (Exception ex)
            {
        ...
        public IActionResult UPDATEDATA(SAC_Mst Data)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
                ...
                Data.UpdatedBy = HttpContext.Session.GetString("UserName");
                Genrate_Query genrate = new Genrate_Query();
                string Query = genrate.GenerateUpdateQuery(Data, "[SAC_Mst]", "ServiceId", Data.ServiceId, "");
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    con.Open();
                    using (SqlCommand cmd = new SqlCommand(Query, con))
                ...
                return Json(new { Success = true, Message = " SAC Setup Updated Successfully..!" });
            }
            catch (Exception ex)
            {
                return StatusCode(500, ex.Message.ToString());
        ...
        public IActionResult DELETE(string Id)
        {
            ...
            try
            {
                ...
                string Query = "Delete from [SAC_Mst] where ServiceId='" + Id + "'";
                List<Dictionary<string, object>> dataList = new List<Dictionary<string, object>>();
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    using (SqlCommand cmd = new SqlCommand(Query, con))
                ...
                return Json(new { success = true, message = " SAC Setup  Deleted Successfully..!" });
            }
            catch (Exception ex)
            {
                return StatusCode(500, ex.Message.ToString());
