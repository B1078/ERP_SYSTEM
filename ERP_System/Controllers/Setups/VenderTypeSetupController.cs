...
{
    public class VenderTypeSetupController : Controller
    {
        public IActionResult VenderTypeSetup()
        {
        ...
        public IActionResult GETDATA()
        {
            try
            {
                ...
                string Query = @"select * from[VendorType_Mst]";
        ...
        public ActionResult POSTDATAExcel(string data)
        {
            ...
            try
            {
                ...
                List<VenderTypeSetup> units = JsonConvert.DeserializeObject<List<VenderTypeSetup>>(data);
                ...
                using (SqlConnection con = new SqlConnection(connectionString))
                {
                    ...
                    foreach (var unit in units)
                    {
                        try
                        {
                            ...
                            string insertQuery = genrate.GenerateInsertQuery(unit, "[VendorType_Mst]", "VendTypeId");
        ...
        public IActionResult POSTDATA(VenderTypeSetup Data)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
                ...
                Genrate_Query genrate = new Genrate_Query();
                string insertQuery = genrate.GenerateInsertQuery(Data, "[VendorType_Mst]", "VendTypeId");
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    con.Open();
                    using (SqlCommand cmd = new SqlCommand(insertQuery, con))
                ...
                return Json(new { Success = true, Message = " Vender Type   Added Successfully..!" });
            }
            catch (SqlException sqlEx)
            {
                return StatusCode(500, "A Vender Type  with this Vender Group already exists. Please use a different Vender Group.");
            }
            catch (Exception ex)
            {
        ...
        public IActionResult UPDATEDATA(VenderTypeSetup Data)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
                ...
                Data.UpdatedBy = HttpContext.Session.GetString("UserName");
                Genrate_Query genrate = new Genrate_Query();
                string Query = genrate.GenerateUpdateQuery(Data, "[VendorType_Mst]", "VendTypeId", Data.VendTypeId, "");
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    con.Open();
                    using (SqlCommand cmd = new SqlCommand(Query, con))
                ...
                return Json(new { Success = true, Message = " Vender Type Updated Successfully..!" });
            }
            catch (Exception ex)
            {
                return StatusCode(500, ex.Message.ToString());
        ...
        public IActionResult DELETE(string Id)
        {
            ...
            try
            {
                ...
                string Query = "Delete from [VendorType_Mst] where VendTypeId='" + Id + "'";
                List<Dictionary<string, object>> dataList = new List<Dictionary<string, object>>();
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    using (SqlCommand cmd = new SqlCommand(Query, con))
                ...
                return Json(new { success = true, message = "Vender Type  Deleted Successfully..!" });
            }
            catch (Exception ex)
            {
                return StatusCode(500, ex.Message.ToString());
