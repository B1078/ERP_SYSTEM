...
{
    public class TaxCombinationSetupController : Controller
    {
        public IActionResult TaxCombinationSetup()
        {
        ...
        public IActionResult GETTAXSETUP()
        {
            try
            {
                ...
                string Query = @" Select * From TaxType_Mst where IsActive ='Y'";
        ...
        public IActionResult GETTAXFORMDATA(string Id)
        {
            try
            {
                string ConnectionString = HttpContext.Session.GetString("ConnectionString");
                string Query = @"select TTM.TaxTypeId, TTM.TaxTypeCode,TTM.TaxTypePer ,'Y' as Active
from TaxType_Mst TTM
inner join TaxCodeForm_Det TCFD on TCFD.TaxTypeId = TTM.TaxTypeId  where TCFD.TaxCodeId='"+ Id + "' union all select TTM.TaxTypeId, TTM.TaxTypeCode,TTM.TaxTypePer ,'N' as Inactive from TaxType_Mst TTM where TTM.TaxTypeId not in (select TCFD.TaxTypeId from  TaxCodeForm_Det TCFD) ";
                List<Dictionary<string, object>> dataList = new List<Dictionary<string, object>>();
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    using (SqlCommand cmd = new SqlCommand(Query, con))
        ...
        [HttpPost]
        public IActionResult POST(TaxCombinationSetup Data)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
                ...
                return Json(new { Success = true, LastId = BPId, Message = " Tax Combination Details Added Successfully..!" });
            }
            catch (SqlException ex)
            {
                ...
                {
                    return StatusCode(500, "This Tax  Code Is Alredy In System Please Define New..!");
                }
                else
                {
                    return StatusCode(500, ex.Message.ToString());
        ...
        public ActionResult POSTDATAExcel(string data)
        {
            ...
            try
            {
                ...
                List<TaxCombinationSetup> units = JsonConvert.DeserializeObject<List<TaxCombinationSetup>>(data);
                ...
                using (SqlConnection con = new SqlConnection(connectionString))
                {
                    ...
                    foreach (var unit in units)
                    {
                        try
                        {
                            ...
                            string insertQuery = genrate.GenerateInsertQuery(unit, "[TaxCode_Mst]", "TaxCodeId");
        ...
        public IActionResult UPDATEDATA(TaxCombinationSetup Data)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
                ...
                Data.UpdatedBy = HttpContext.Session.GetString("UserName");
                Genrate_Query genrate = new Genrate_Query();
                string Query = genrate.GenerateUpdateQuery(Data, "[TaxCode_Mst]", "TaxCodeId", Data.TaxCodeId, "");
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    con.Open();
                ...
                return Json(new { Success = true, LastId = Data.TaxCodeId, Message = " Tax Combination  Details Updated Successfully..!" });
            }
            catch (Exception ex)
            {
                return StatusCode(500, ex.Message.ToString());
        ...
        public IActionResult ADDFORMDT([FromBody] List<TaxCombibnationForm> Data)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
                ...
                {
                    if (item.TaxCodeFormId != "" && item.TaxCodeFormId != null && item.TaxCodeFormId != "0")
                    {
                      
                        Genrate_Query genrate = new Genrate_Query();
                        string Query = genrate.GenerateUpdateQuery(item, "[TaxCodeForm_Det]", "TaxCodeFormId", item.TaxCodeFormId, "");
                        if (item.TaxTypeId != null || item.TaxTypeId != "")
                        {
                            using (SqlConnection con = new SqlConnection(ConnectionString))
                            {
                                con.Open();
        ...
        public IActionResult DELETE(string Id)
        {
            ...
            try
            {
                ...
                string Query = "Delete from [TaxCode_Mst] where TaxCodeId='" + Id + "'";
                List<Dictionary<string, object>> dataList = new List<Dictionary<string, object>>();
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    using (SqlCommand cmd = new SqlCommand(Query, con))
                ...
                return Json(new { success = true, message = "Tax Combination  Details Deleted Successfully..!" });
            }
            catch (Exception ex)
            {
                return StatusCode(500, ex.Message.ToString());
