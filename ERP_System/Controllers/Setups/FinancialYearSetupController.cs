...
namespace ERP_System.Controllers.Setups
{
    ...
    {
        public IActionResult FinancialYearSetup()
        {
        ...
        public IActionResult GETDATA()
        {
            try
            {
                ...
                string Query = @"
select FinyrId  , Finyr,
CONVERT(varchar(10), stdt, 105) AS Stdt,
CONVERT(varchar(10), Enddt , 105) AS Enddt,
IsActive ,CreatedBy ,CreateDate ,CreateTS ,UpdatedBy ,UpdateDate ,UpdateTS ,Fisyr ,Pst ,FinyrStNum 
from [Finyr_mst]
";
        ...
        public IActionResult POSTDATA(Fincyear_mst Data)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
                ...
                string insertQuery = genrate.GenerateInsertQuery(Data, "[Finyr_mst]", "FinyrId");
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    con.Open();
                ...
                return Json(new { Success = true, Message = " Financial Year Setup  Added Successfully..!" });
            }
            catch (SqlException sqlEx)
            {
                return StatusCode(500, "A Financial Year Setup with this Financial Year already exists. Please use a different Financial Year.");
            }
            catch (Exception ex)
            {
        ...
        public ActionResult POSTDATAExcel(string data)
        {
            ...
            try
            {
                ...
                List<Fincyear_mst> units = JsonConvert.DeserializeObject<List<Fincyear_mst>>(data);
                ...
                using (SqlConnection con = new SqlConnection(connectionString))
                {
                    ...
                    foreach (var unit in units)
                    {
                        try
                        {
                            ...
                            string insertQuery = genrate.GenerateInsertQuery(unit, "[Finyr_mst]", "FinyrId");
        ...
        public IActionResult UPDATEDATA(Fincyear_mst Data)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
                ...
                Data.UpdatedBy = HttpContext.Session.GetString("UserName");
                Genrate_Query genrate = new Genrate_Query();
                string Query = genrate.GenerateUpdateQuery(Data, "[Finyr_mst]", "FinyrId", Data.FinyrId, "");
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    con.Open();
                    using (SqlCommand cmd = new SqlCommand(Query, con))
        ...
        public IActionResult DELETE(string Id)
        {
            ...
            try
            {
                ...
                string Query = "Delete from [Finyr_mst] where FinyrId='" + Id + "'";
                List<Dictionary<string, object>> dataList = new List<Dictionary<string, object>>();
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    using (SqlCommand cmd = new SqlCommand(Query, con))
                ...
                return Json(new { success = true, message = "Financial Setup Deleted Successfully..!" });
            }
            catch (Exception ex)
            {
                return StatusCode(500, ex.Message.ToString());
