...
{
    public class TaxTypeSetupController : Controller
    {
        public IActionResult TaxTypeSetup()
        {
        ...
        public IActionResult GETDATA()
        {
            try
            {
                ...
                string Query = @"select * from[TaxType_Mst]";
        ...
        public ActionResult POSTDATAExcel(string data)
        {
            ...
            try
            {
                ...
                List<Taxtype_mst> units = JsonConvert.DeserializeObject<List<Taxtype_mst>>(data);
                ...
                using (SqlConnection con = new SqlConnection(connectionString))
                {
                    ...
                    foreach (var unit in units)
                    {
                        try
                        {
                            ...
                            string insertQuery = genrate.GenerateInsertQuery(unit, "[TaxType_Mst]", "TaxTypeId");
        ...
        public IActionResult POSTDATA(Taxtype_mst Data)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
                ...
                Genrate_Query genrate = new Genrate_Query();
                string insertQuery = genrate.GenerateInsertQuery(Data, "[TaxType_Mst]", "TaxTypeId");
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    con.Open();
                    using (SqlCommand cmd = new SqlCommand(insertQuery, con))
            ...
            {
                return StatusCode(500, "A Tax Type with this Tax Type Code already exists. Please use a different Tax Type Code.");
            }
            catch (Exception ex)
            {
        ...
        public IActionResult UPDATEDATA(Taxtype_mst Data)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
                ...
                Data.UpdatedBy = HttpContext.Session.GetString("UserName");
                Genrate_Query genrate = new Genrate_Query();
                string Query = genrate.GenerateUpdateQuery(Data, "[TaxType_Mst]", "TaxTypeId", Data.TaxTypeId, "");
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    con.Open();
                    using (SqlCommand cmd = new SqlCommand(Query, con))
        ...
        public IActionResult DELETE(string Id)
        {
            ...
            try
            {
                ...
                string Query = "Delete from [TaxType_Mst] where TaxTypeId='" + Id + "'";
                List<Dictionary<string, object>> dataList = new List<Dictionary<string, object>>();
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    using (SqlCommand cmd = new SqlCommand(Query, con))
