    ...
    public class ItemPriceListController : Controller
    {
        ...
        public IActionResult GET()
        {
            try
            {
                ...
                string Query = @"Select * ,T1.PListName,T2.ItemName From Price_List_Det T0
Inner Join  Price_List_Mst T1 On T0.PListId =T1.PListId
Inner Join  Item_Mst T2 On T0.ItemId =T2.ItemId";
        ...
        public IActionResult GETBASEPRICE(string ItemId)
        {
            try
            {
                string ConnectionString = HttpContext.Session.GetString("ConnectionString");
                string Query = @" select PLD.BasePrice 
from Price_List_Det PLD
inner join Price_List_Mst PLM on PLD.PListId = PLM.PListId and PLM.IsActive ='Y'
where PLM.IsDefaultPrice ='Y'
and PLD.ItemId = '" + ItemId + "'";
                List<Dictionary<string, object>> dataList = new List<Dictionary<string, object>>();
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    using (SqlCommand cmd = new SqlCommand(Query, con))
        ...
        [HttpPost]
        public IActionResult POSTDATA(ItemPriceList Data)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
                ...
                string insertQuery = genrate.GenerateInsertQuery(Data, "[Price_List_Det]", "PListDetId");
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    con.Open();
                ...
                return Json(new { Success = true, Message = "Price List  Added Successfully..!" });
            }
            catch (Exception ex)
            {
                return StatusCode(500, ex.Message.ToString());
                ...
                using (SqlConnection con = new SqlConnection(connectionString))
                {
                    ...
                    foreach (var unit in units)
                    {
                        try
                        {
                            ...
                            string insertQuery = genrate.GenerateInsertQuery(unit, "[Price_List_Det]", "PListDetId");
        ...
        public IActionResult UPDATEDATA(ItemPriceList Data)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
                ...
                Data.UpdatedBy = HttpContext.Session.GetString("UserName");
                Genrate_Query genrate = new Genrate_Query();
                string Query = genrate.GenerateUpdateQuery(Data, "[Price_List_Det]", "PListDetId", Data.PListDetId, "");
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    con.Open();
                ...
                return Json(new { Success = true, Message = "Price List Updated Successfully..!" });
            }
            catch (Exception ex)
            {
                return StatusCode(500, ex.Message.ToString());
        ...
        public IActionResult DELETE(string Id)
        {
            ...
            try
            {
                ...
                string Query = "Delete from [Price_List_Det] where PListDetId='" + Id + "'";
                List<Dictionary<string, object>> dataList = new List<Dictionary<string, object>>();
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    using (SqlCommand cmd = new SqlCommand(Query, con))
                ...
                return Json(new { success = true, message = "Price List  Deleted Successfully..!" });
            }
            catch (Exception ex)
            {
                return StatusCode(500, ex.Message.ToString());
