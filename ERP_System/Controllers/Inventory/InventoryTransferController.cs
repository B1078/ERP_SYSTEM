...
namespace ERP_System.Controllers.Inventory
{
    ...
    {
        public IActionResult InventoryTransfer()
        {
        ...
        public IActionResult GETATACHMENT(string Id)
        {
            ...
            {
                string ConnectionString = HttpContext.Session.GetString("ConnectionString");
                string Query = @"Select * FROM Attach_Det where DocId='" + Id + "' and DocType ='IT'";
                List<Dictionary<string, object>> dataList = new List<Dictionary<string, object>>();
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    using (SqlCommand cmd = new SqlCommand(Query, con))
        ...
        public IActionResult GETITEMDET(string ID, string Flag)
        {
            ...
            {
                string ConnectionString = HttpContext.Session.GetString("ConnectionString");
                string Query = @"select * from [InvTransfer_det]";
                List<Dictionary<string, object>> dataList = new List<Dictionary<string, object>>();
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    using (SqlCommand cmd = new SqlCommand(Query, con))
            ...
            try
            {
                ...
                switch (Flag)
                {
                    ...
                    case "PBA":
                        ...
                        break;
                    case "ITR":
                        Query = @"Select * From InvTrans_Req where  Status='O' AND BPId ='" + BPId + "'";
                        break;
                    case "GRPO":
        ...
        [HttpPost]
        public IActionResult POSTUPDATE(InvtTrn_Mst item)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            string Lastid = "";
            try
            {
                ...
                if (item.InvTransId != "" && item.InvTransId != null)
                {
                    ...
                    string Query = genrate.GenerateUpdateQuery(item, "[InvTransfer]", "InvTransId", item.InvTransId, "");
                    ...
                    Lastid = item.InvTransId;
                    return Json(new { Success = true, LastId = Lastid, Message = "Inventory Transfer Updated Successfully..!" });
                }
                else
                {
                    item.InvTransId = null;
                    item.UpdateDate = DateOnly.FromDateTime(DateTime.Now.Date);
                    item.UpdateTS = TimeOnly.FromTimeSpan(new TimeSpan(ticks));
                    item.UpdatedBy = HttpContext.Session.GetString("UserName");
                    ...
                    if (item.BPId != null || item.BPId != "")
                    {
                        string insertQuery = genrate.GenerateInsertQuery(item, "[InvTransfer]", "InvTransId");
                        using (SqlConnection con = new SqlConnection(ConnectionString))
                        {
                            con.Open();
                            using (SqlCommand cmd = new SqlCommand(insertQuery, con))
                            {
                                cmd.CommandTimeout = 300;
                                cmd.ExecuteNonQuery();
                                cmd.CommandText = "SELECT MAX(InvTransId) AS InvTransId FROM InvTransfer;";
                                SqlDataReader rdr = cmd.ExecuteReader();
                                {
                                    while (rdr.Read())
                                    {
                                        Lastid = rdr["InvTransId"].ToString();
                                    }
                                }
                            }
...
FROM InvTransfer 
inner join Finyr_mst on InvTransfer.FinyrId = Finyr_mst.FinyrId 
and Finyr_mst.IsActive ='Y'   where DraftFlag='" + flag + "'; ";
        ...
        public IActionResult ADDITEMDETAILS([FromBody] List<InvTransDet_Mst> Data)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
                ...
                foreach (var item in Data)
                {
                    ...
                    if (item.InvTransDetId != "" && item.InvTransDetId != null)
                    {
                        if (item.RowStatus == "O")
                        {
                        ...
                        item.UpdatedBy = HttpContext.Session.GetString("UserName");
                        Genrate_Query genrate = new Genrate_Query();
                        string Query = genrate.GenerateUpdateQuery(item, "[InvTransfer_det]", "InvTransDetId", item.InvTransDetId, "");
                        if (item.InvTransId != null || item.InvTransId != "")
                        {
                            using (SqlConnection con = new SqlConnection(ConnectionString))
                            {
                                con.Open();
                    ...
                    else
                    {
                        ...
                        if (item.InvTransId != null || item.InvTransId != "")
                        {
                            string insertQuery = genrate.GenerateInsertQuery(item, "[InvTransfer_det]", "InvTransDetId");
                            using (SqlConnection con = new SqlConnection(ConnectionString))
                            {
                                con.Open();
                                using (SqlCommand cmd = new SqlCommand(insertQuery, con))
                ...
                return Json(new { Success = true, Message = "Inventory Transfer  Item Details Added  Successfully..!" });
            }
            catch (Exception ex)
            {
                return StatusCode(500, ex.Message.ToString());
        ...
        public IActionResult DELETEMAINDATA(string Id)
        {
            ...
            try
            {
                ...
                string Query = "Delete from [InvTransfer] where InvTransId='" + Id + "'";
                List<Dictionary<string, object>> dataList = new List<Dictionary<string, object>>();
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    using (SqlCommand cmd = new SqlCommand(Query, con))
        ...
        public IActionResult DELETEDETAILS(string Id)
        {
            ...
            try
            {
                ...
                string Query = "Delete from [InvTransfer_det] where InvTransDetId ='" + Id + "'";
                List<Dictionary<string, object>> dataList = new List<Dictionary<string, object>>();
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    using (SqlCommand cmd = new SqlCommand(Query, con))
