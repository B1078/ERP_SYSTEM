    ...
    public class MenuMasterController : Controller
    {
        ...
        public IActionResult GETMENUDATA()
        {
            try
            {
                ...
                string Query = @"Select * From [Menu_Mst]";
...
where T0.TableName ='Menu_Mst' And T1.IsMandatory='Y'";
                List<Dictionary<string, object>> dataList = new List<Dictionary<string, object>>();
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    using (SqlCommand cmd = new SqlCommand(Query, con))
        ...
        [HttpPost]
        public IActionResult POSTDATA(MenuMst Data)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
                ...
                string insertQuery = genrate.GenerateInsertQuery(Data, "[Menu_Mst]", "MenuId");
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    con.Open();
        ...
        public IActionResult UPDATEDATA(MenuMst Data)
        {
            string ConnectionString = HttpContext.Session.GetString("ConnectionString");
            try
            {
                ...
                Data.UpdatedBy = HttpContext.Session.GetString("UserName");
                Genrate_Query genrate = new Genrate_Query();
                string Query = genrate.GenerateUpdateQuery(Data, "[Menu_Mst]", "MenuId", Data.MenuId, "");
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    con.Open();
        ...
        public IActionResult DELETE(string Id)
        {
            ...
            try
            {
                ...
                string Query = "Delete from [Menu_Mst] where MenuId='" + Id + "'";
                List<Dictionary<string, object>> dataList = new List<Dictionary<string, object>>();
                using (SqlConnection con = new SqlConnection(ConnectionString))
                {
                    using (SqlCommand cmd = new SqlCommand(Query, con))
                ...
                return Json(new { success = true, message = "Manu Deleted Successfully..!" });
            }
            catch (Exception ex)
            {
                return StatusCode(500, ex.Message.ToString());
